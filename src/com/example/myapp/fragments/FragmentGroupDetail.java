...
/**
 * 月明细页面,listView每条明细可展开,里面会显示对应的日明细
 * 
 * @author Engle
 * 
 */
	...
	protected List<List<DayDetailItem>> loadChildViewData(
		...
		for (int i = 0; i < groupDetailItem.getDateRangeList().size(); i++) {
			...
			Cursor cursor = dbHelper
					.getReadableDatabase()
					.rawQuery(
							"select amount,type,date,accountType from detail_record where date > ? and date < ?",
							new String[] {
			...
			while (cursor.moveToNext()) {
				try {
					...
					dayDetailItem.setDayDetailAccountType(new String(cursor
							...
							"gb2312"));
					dayDetailItem.setDayDetailConsumeAmount(cursor
